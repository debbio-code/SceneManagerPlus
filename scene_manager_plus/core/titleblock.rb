  ...
  module Core
    ...
    module TitleBlock
      ...
      PS_SCRIPT = <<~PS
        ...
        # Indici: 0 cliente, 1 fase(+scala), 2 tavola, 3 progetto, 4 dati, 5 logo.
        # ATTENZIONE: la SCALA non e' una colonna a se'. Vive nella meta'
        # INFERIORE del box 1 (fase), che senza scala e' occupata dalla riga
        # OGGETTO. Il cartiglio dell'export a serie resta quindi identico a
        # prima quando la scena non ha una scala.
        ...
        foreach ($item in $cfg.items) {
          ...
          for ($i=0; $i -lt $widths.Length - 1; $i++) {
            ...
            # Il divisore tra box 0 (CLIENTE/OGGETTO) e box 1 (FASE) e'
            # PARZIALE (solo riga superiore) quando non c'e' scala, perche' la
            # riga OGGETTO prosegue sotto il box fase. Con la scala invece la
            # meta' inferiore del box 1 e' una cella vera e propria, quindi il
            # divisore va a tutta altezza.
            if ($i -eq 0 -and -not $hasScala) {
          ...
          # OGGETTO: occupa la larghezza unificata box 0 + box 1 SOLO quando
          # sotto il box fase non c'e' la cella della scala.
          if ($hasScala) {
            $rowMaxW_ogg = [single]($widths[0] - 2 * $pad)
          } else {
          ...
          # ---------- Box 1: Fase di progetto (top) / SCALA (bottom) ----------
